...
def _result(memory_id: str, content: str, score: float, *, entities: list[str] | None = None):
    return SimpleNamespace(
        memory=SimpleNamespace(id=memory_id, content=content, metadata={"virtual_path": "/memories/topic"}),
        score=score,
...
def test_inject_tools_respects_provider_modes_and_existing_tools() -> None:
    ...
    assert MEMORY_TOOL_NAMES <= {
        tool.get("function", {}).get("name", "") for tool in openai_tools
    }
    ...
    assert MEMORY_TOOL_NAMES <= {
        tool.get("function", {}).get("name", "") for tool in generic_tools
    }
...
def test_extract_tool_calls_names_ids_and_inputs_cover_all_formats() -> None:
    ...
    anthropic_response = {"content": [{"type": "tool_use", "id": "a1", "name": "memory_search", "input": {"q": 1}}]}
    openai_response = {
        "choices": [
            {
                "message": {
                    "tool_calls": [
                        {
                            "id": "o1",
                            "function": {"name": "memory_save", "arguments": json.dumps({"content": "x"})},
                        }
                    ]
                }
            }
    ...
    gemini_response = {
        "candidates": [{"content": {"parts": [{"functionCall": {"name": "memory_delete", "args": {"id": 1}}}]}}]
    }
    generic_response = {
        "content": [{"type": "tool_use", "id": "g1", "name": "memory_update", "input": {"x": 1}}],
        "choices": [{"message": {"tool_calls": [{"id": "g2", "function": {"name": "other", "arguments": "{bad"}}]}}],
    }
    ...
    assert adapter._extract_tool_calls(gemini_response, "gemini")[0]["functionCall"]["name"] == "memory_delete"
    ...
    assert adapter._get_tool_name({"functionCall": {"name": "memory_delete"}}, "gemini") == "memory_delete"
    assert adapter._get_tool_name({"function": {"name": "memory_update"}}, "generic") == "memory_update"

    assert adapter._get_tool_id({"id": "a1"}, "anthropic") == "a1"
    assert adapter._get_tool_id({"id": "o1"}, "openai") == "o1"
    assert adapter._get_tool_id({"functionCall": {"name": "memory_search"}}, "gemini") == "memory_search"
...
def test_has_memory_tool_calls_and_format_tool_result_cover_variants() -> None:
    ...
    assert adapter.has_memory_tool_calls({"choices": [{"message": {"tool_calls": []}}]}, "openai") is False
...
async def test_handle_tool_calls_formats_results_and_skips_non_memory_tools(monkeypatch) -> None:
    ...
    assert results == [{"role": "tool", "tool_call_id": "1", "content": "custom:memory_save:user-1"}]
    ...
    assert native_results == [{"type": "tool_result", "tool_use_id": "n1", "content": "native-result"}]
...
async def test_native_tool_commands_and_custom_tools_cover_success_and_errors() -> None:
    ...
    assert "Please provide a search query" in await adapter._native_view({"path": "/memories/search/"}, "user-1")
    assert "Found 2 memories matching 'alpha'" in await adapter._native_view({"path": "/memories/search/alpha"}, "user-1")
    assert "Memory System (2 memories stored)" in await adapter._native_view({"path": "/memories"}, "user-1")

    assert await adapter._native_create({"path": "/memories/topic.txt", "file_text": "saved text"}, "user-1") == (
        "File created successfully at: /memories/topic.txt"
    )
    assert await adapter._native_create({"path": "/memories/topic.txt"}, "user-1") == "Error: file_text is required"
    ...
    assert await adapter._native_update({"new_str": "updated"}, "user-1") == "Error: old_str is required"
    ...
    assert await adapter._native_delete({"path": "/memories/topic"}, "user-1") == "Successfully deleted /memories/topic"
    ...
    assert await adapter._execute_native_tool({"command": "view"}, "user-1") == "Error: Memory backend not initialized"
...
async def test_custom_tool_execution_and_lifecycle_cover_branches(monkeypatch) -> None:
    ...
    saved = json.loads(await adapter._execute_save({"content": "remember this", "importance": 0.9}, "user-1"))
    ...
    searched = json.loads(await adapter._execute_search({"query": "remember", "top_k": 3}, "user-1"))
    ...
    assert json.loads(await adapter._execute_update({"new_content": "x"}, "user-1"))["error"] == "memory_id is required"
    assert json.loads(await adapter._execute_update({"memory_id": "m1"}, "user-1"))["error"] == "new_content is required"
    ...
    assert json.loads(await adapter._execute_delete({}, "user-1"))["error"] == "memory_id is required"

    adapter._backend = None
    assert json.loads(await adapter._execute_custom_tool("memory_save", {"content": "x"}, "user-1"))["error"] == (
        "Memory backend not initialized"
    )

    adapter._backend = backend
    assert json.loads(await adapter._execute_custom_tool("unknown", {}, "user-1"))["error"] == "Unknown tool: unknown"

    async def boom(input_data, user_id):  # noqa: ANN001, ANN201
        raise RuntimeError("boom")

    monkeypatch.setattr(adapter, "_execute_save", boom)
    failed = json.loads(await adapter._execute_custom_tool("memory_save", {"content": "x"}, "user-1"))
